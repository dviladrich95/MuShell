...
def get_scale_cents_and_root(scale_name):
    ...
    with open(os.path.join(scale_directory, scale_name), "r") as scale_file:
        ...
        for note in scale_list_noconv:
            if '/' in note:
                scale_list.append(math.log2(
                    float(Fraction(note))) * 1200)  # numbers expressed as ratios need to be converted into cents first
            else:
...
def qbox_list2midi(qbox_list, root_note, exp_scale_list, midi_str, forced_duration=0):
    ...
    for note, time, duration, _ in qbox_list:
        midi_file.addNote(track, channel, note + root_note, time, duration,
                          volume)  # time and duration measured in beats
